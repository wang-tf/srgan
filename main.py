...
def arg_parse():
    ...
    parser.add_argument('--have_enough_memory', type=bool, default=False, help='if you have enough memory, the whole images will preload')
...
def train(have_enough_memory):
    # # create folders to save result images and trained model
    save_dir_ginit, save_dir_gan, checkpoint_dir = pre_make_dir()

    # ##====================== PRE-LOAD DATA ===========================###
    train_hr_img_list, train_lr_img_list, valid_hr_img_list, valid_lr_img_list = pre_load_data_list()
...
def pre_make_dir():
    ...
    return save_dir_ginit, save_dir_gan, checkpoint_dir


def pre_load_data_list():
    train_hr_img_list = sorted(tl.files.load_file_list(path=config.TRAIN.hr_img_path, regx='.*.png', printable=False))
    train_lr_img_list = sorted(tl.files.load_file_list(path=config.TRAIN.lr_img_path, regx='.*.png', printable=False))
    valid_hr_img_list = sorted(tl.files.load_file_list(path=config.VALID.hr_img_path, regx='.*.png', printable=False))
    valid_lr_img_list = sorted(tl.files.load_file_list(path=config.VALID.lr_img_path, regx='.*.png', printable=False))
    return train_hr_img_list, train_lr_img_list, valid_hr_img_list, valid_lr_img_list
